...
/**
 * 根据字符串变量lrc的值,计算出一个数组,数组的每一项是一个对像,对象中记录了以下信息
...
function createLrcArray() {
  ...
  function createLrcObject(str) {
    var parts = str.split(']')
    var words = parts[1] // 第二部分是歌词
    var time = parts[0] // 第一部分是时间
    time = time.replace('[', '') // 去掉左中括号
    var timeParts = time.split(':') // 分割时间
    var minute = parseInt(timeParts[0])
    var second = parseInt(timeParts[1])
 ...
 */
 ...
 function getCurrentIndex() {
   var playTime = audio.currentTime + config.offset // audio元素当前播放时间
  ...
  */
 function setCurrent() {
   var index = getCurrentIndex() // 得到当前播放的歌词是数组的第几项下标
   setUlMarginTop()
   setLiActive()
